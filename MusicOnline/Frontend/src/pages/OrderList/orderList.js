...
const OrderList = () => {
    ...
    const handleOkUser = async (values) => {
        ...
        try {
                ...
                return axiosClient.post("/artist", categoryList).then(response => {
    ...
    const handleUpdateCategory = async (values) => {
        ...
        setLoading(true);
            try {
                ...
                return axiosClient.put("/artist/" + id, categoryList).then(response => {
                    if (response === undefined) {
                        notification["error"]({
                            message: `Thông báo`,
                            description:
                                'Chỉnh sửa nghệ sĩ thất bại',
                        });
                    }
                    else {
                        notification["success"]({
    ...
    const handleDeleteCategory = async (id) => {
        setLoading(true);
        try {
            await productApi.deleteArtist(id).then(response => {
                if (response === undefined) {
                    notification["error"]({
                        message: `Thông báo`,
                        description:
                            'Xóa nghệ sĩ thất bại',
    ...
    const handleEditCategory = (id) => {
        ...
        (async () => {
            try {
                const response = await productApi.getDetailArtist(id);
                ...
                <div>
                    <Row>
                        <Button
                            ...
                            onClick={() => handleEditCategory(record.ArtistID)}
                        >{"Chỉnh sửa"}
                        </Button>
                        <div
                            style={{ marginLeft: 10 }}>
                            <Popconfirm
                                title="Bạn có chắc chắn xóa nghệ sĩ này?"
                                onConfirm={() => handleDeleteCategory(record.ArtistID)}
                                okText="Yes"
                                cancelText="No"
                            >
                                <Button
                            ...
                            >
                                <Row>
                                    ...
                                    <Col span="6">
                                        <Row justify="end">
                                            <Space>
                                                <Button onClick={showModal} icon={<PlusOutlined />} style={{ marginLeft: 10 }} >Tạo nghệ sĩ</Button>
                                            </Space>
                                        </Row>
                                    </Col>
                                </Row>
